...
_DOCKERIGNORE_BEGIN = "# container-magic:begin (generated, do not edit)"
_DOCKERIGNORE_END = "# container-magic:end"


def update_dockerignore(path: Path, workspace_name: str):
    """Update .dockerignore with a managed deny-by-default section.

    The build context only needs the workspace and cached assets. Everything
    else (outputs, data folders, .git, scripts) is excluded. A managed
    section between markers is regenerated on every cm update; user additions
    ...
    for i, line in enumerate(lines):
        if line.strip() == _DOCKERIGNORE_BEGIN:
            begin_idx = i
        elif line.strip() == _DOCKERIGNORE_END:
            end_idx = i

    if begin_idx is not None and end_idx is not None and end_idx > begin_idx:
        # Replace existing managed section
        new_lines = lines[:begin_idx] + managed_lines + lines[end_idx + 1 :]
    else:
        # No managed section yet - prepend it, keep existing content below
        new_lines = managed_lines + [""] + lines

    result = "\n".join(new_lines)
    if not result.endswith("\n"):
        result += "\n"
    dockerignore_path.write_text(result)
...
def init(
    ...
    update_dockerignore(path, config.names.workspace)
